...
- Лениво добавляет подробное объяснение через agent-explainer после нажатия «Подробнее».
...
8. Backend сохраняет результат ранжирования в PostgreSQL без ожидания объяснений.
9. Frontend получает статус и показывает вкладки с решениями.
10. При нажатии «Подробнее» explanation-agent объясняет только выбранную карточку; результат кэшируется.
...
Основная обработка выполняет один LLM-вызов для извлечения intent. Дополнительный LLM-вызов
выполняется только для карточки, у которой пользователь нажал «Подробнее». Полученное объяснение
сохраняется в PostgreSQL, поэтому повторное открытие той же карточки не обращается к LLM.
