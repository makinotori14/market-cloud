  ...
  for (const [index, prompt] of prompts.entries()) {
    const requestResult = await pool.query(
      `SELECT id, created_at
       ...
       WHERE prompt = $1 AND status = 'completed'
       ORDER BY created_at DESC
       LIMIT 3`,
      [prompt],
    );

    if (requestResult.rows.length !== 3) {
      throw new Error(`Expected 3 completed runs for prompt ${index + 1}, got ${requestResult.rows.length}`);
    }

    const runs = [];
    for (const request of [...requestResult.rows].reverse()) {
      const recommendationResult = await pool.query(
        `SELECT provider, title, final_score
